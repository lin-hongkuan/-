...
function App() {
  ...
  const [aiEnabled, setAiEnabled] = useState(true); // Always true internally now, meaning "game systems enabled"
  ...
  const fetchNewGoal = useCallback(async () => {
    ...
    // Call local service (instant/simulated delay)
  ...
  useEffect(() => {
    ...
    addNewsItem({ id: Date.now().toString(), text: "欢迎来到天空之城。地形生成完毕。", type: 'positive' });
    
    // Start first goal immediately
    fetchNewGoal();
  ...
  const handleTileClick = useCallback((x: number, y: number) => {
    ...
    if (currentTile.buildingType === BuildingType.None) {
      if (currentStats.money >= buildingConfig.cost) {
        ...
        addNewsItem({id: Date.now().toString() + Math.random(), text: `资金不足，无法建造${buildingConfig.name}。`, type: 'negative'});
      }
    }
